...
<h1>会员注册管理系统</h1>
        ...
        if(isset($_SESSION['loggedUsername']) && $_SESSION['loggedUsername'] <> ''){
            ?>
            <div class="logged">当前用户：<?php echo $_SESSION['loggedUsername'];?><?php if($_SESSION['isAdmin']){?> <span style="color: crimson"> ， 欢迎管理员！</span><?php }?><span class="logout"><a href="logout.php">注销登录</a></span></div>
        <?php
        }
        $id = $_GET['id'] ?? 1;//判断跳转后的页面是哪一个; 三元运算符
        ?>
